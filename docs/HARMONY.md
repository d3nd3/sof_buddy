...
- Requires `SOFBUDDY_ENABLE_CALLSITE_LOGGER` define to be active
- Helps developers discover all call sites for a hook
- Files are written to `sof_buddy/func_parents/*.json` at runtime (next to game executable)
- Files can be manually copied to `rsrc/func_parents/` for reference in the repository
...
2. **Update caller detection code:**
   - Copy generated JSON files to `rsrc/func_parents/` (optional, for repository reference)
   - Run `python rsrc/update_callers_from_parents.py` to validate against existing code
   - Run `python rsrc/update_callers_from_parents.py --write` to auto-update `caller_from.cpp`
   - The script reads JSON files, extracts parent RVAs, and inserts missing `case` statements into the appropriate switch blocks in `caller_from.cpp`

3. **Runtime loading (optional):**
   - If JSON files exist in `rsrc/func_parents/`, they are loaded on startup via `ParentRecorder::loadExistingData()`
   - Prevents re-recording known parent callers across runs, speeding up subsequent executions
...
# Update caller detection code
python rsrc/update_callers_from_parents.py --write
```

**Script Details:**
- `rsrc/update_callers_from_parents.py` maps child hook names (e.g., `"glVertex2f"`) to functions in `caller_from.cpp` (e.g., `getVertexCallerFromRva()`)
- Supports module-aware functions with nested switch statements by module
- Only adds missing cases; never removes existing ones
- Validates that discovered RVAs match existing funcmap data
