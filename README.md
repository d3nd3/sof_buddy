...
| `_sofbuddy_font_scale_auto`      | 1                                                         | Auto font scaling from resolution (`vid_h / 480`). `1` = on (default), `0` = off — use `_sofbuddy_font_scale` for a fixed multiplier. Updates on `vid_restart` / resolution changes. F12 → UI Scale → **Auto Font Scale**. Legacy configs with `_sofbuddy_font_scale -1` are migrated to auto on + scale `1` on first load. |
| `_sofbuddy_font_scale`           | 1                                                         | Manual font scale multiplier (`0.25` … `8` in 0.25 steps). Used when `_sofbuddy_font_scale_auto` is `0`. F12 → UI Scale shows this list only when auto is off. |
| `_sofbuddy_hud_scale_auto`       | 1                                                         | Auto HUD scaling from resolution (`vid_h / 480`). `1` = on (default), `0` = off — use `_sofbuddy_hud_scale` for a fixed multiplier. Same ratio as font auto mode. F12 → UI Scale → **Auto HUD Scale**. Legacy `_sofbuddy_hud_scale -1` is migrated like font auto. |
| `_sofbuddy_hud_scale`            | 1                                                         | Manual HUD scale multiplier (`0.25` … `8` in 0.25 steps). Used when `_sofbuddy_hud_scale_auto` is `0`. F12 → UI Scale shows this list only when auto is off. |
| `_sofbuddy_scale_round_auto`     | 0                                                         | Snap auto font/HUD scale to round steps instead of a continuous ratio. `1` = on, `0` = off. F12 → UI Scale → **Auto Round Scale**. |
| `_sofbuddy_scale_round_ratio`    | 0.25                                                      | Snap increment when `_sofbuddy_scale_round_auto` is on (`0.05`, `0.1`, `0.125`, `0.25`, `0.5`, `1`). F12 → UI Scale → **Auto Round Step**. Also applies to cinematic image scaling when auto round is enabled. |
...
| `_sofbuddy_loading_lock_input`   | 0                                                         | Loading menu input lock mode (`1` locked, `0` unlocked)                                                                                                                                                                                                                                                                                                                                        |
